...
hidden_units_num=10
chromosome_len = (20+1) * hidden_units_num + (hidden_units_num+1)*5
...
def select_parents(population,population_fitness):
    ...
    a=population[np.argmax(population_fitness)]
    return a,b,c
...
def crossover(a,b,c,offspring,F,crossover_rate,isMutation=False):
    ...
    index = random.randrange(chromosome_len) #ensure at least one crossover
...
def experiment(crossover_rate,F,enemy,round):
    ...
    '''
    experiment_name = '2_enemy_'+str(enemy)+'_round_'+str(round)
    if not os.path.exists('experiments_data/'+experiment_name):
        os.makedirs('experiments_data/'+experiment_name)
    
    env = Environment(experiment_name='experiments_data/'+experiment_name,
                  enemies=enemy,
                  playermode="ai",
    ...
    for g in range(1,max_generation):
        ...
        if population_fitness[best] <= current_best_fit:
            ...
            notimproved = 0 
        if notimproved >= 10:
            file_aux  = open('experiments_data/'+experiment_name+'/results.txt','a')
            file_aux.write('\n start mutation')
            file_aux.close()
...
toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_float, chromosome_len)
